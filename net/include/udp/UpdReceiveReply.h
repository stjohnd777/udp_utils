...
#include "net.h"
        ...
        void UpdReceiveReplyOne(std::string host, unsigned short port, std::function<Res *(Req *)> handler) {
            ...
            try {
                ...
                // listening for incoming request from a sender
                char rcv_buf[sizeof(Req)];
                boost::asio::ip::udp::endpoint senders_endpoint;
                std::size_t bytes_received = socket.receive_from(boost::asio::buffer(rcv_buf), senders_endpoint);
                // assert bytes_received = M

                // deserialize client request
                auto req = lm::spp::DeSerialize<Req>(rcv_buf);
